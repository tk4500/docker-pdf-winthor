...
export default function Dashboard() {
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const unfinishedJobs = response.data.items.filter(
        job => job.status_global !== 'ENVIADO_WINTHOR' && job.status_global !== 'CANCELADO'
      );
  ...
  const getStatusColor = (status) => {
    ...
    if (status === 'AGUARDANDO_APROVACAO') return 'bg-yellow-100 text-yellow-800';
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <nav className="bg-white shadow-sm px-6 py-4 flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <h1 className="text-xl font-bold text-gray-800">Winthor PDF Parser</h1>
        </div>
        <div className="flex items-center space-x-4">
          <span className="text-sm text-gray-600">
            Olá, <strong>{username}</strong> ({role})
          </span>
          <button onClick={handleLogout} className="text-gray-500 hover:text-red-600 flex items-center text-sm">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-semibold text-gray-800">Pedidos em Andamento</h2>
          <button 
            onClick={fetchJobs} 
                ...
                <li key={job.id}>
                  <div className="px-4 py-4 sm:px-6 hover:bg-gray-50 transition-colors cursor-pointer">
                    <div className="flex items-center justify-between">
                      <p className="text-sm font-medium text-blue-600 truncate">{job.nome_arquivo}</p>
                    ...
                    <div className="mt-2 sm:flex sm:justify-between">
                      ...
                      <div className="mt-2 flex items-center text-sm text-gray-500 sm:mt-0">
                        <p>
                          Criado em: {new Date(job.data_criacao).toLocaleString('pt-BR')}
                        </p>
                      </div>
                    </div>
                  </div>
